...
generated: { by: codex, at: 2026-09-11T03:58:16Z }
status: draft
---
...
아래는 macOS / Linux용 예시다. Windows에서는 CLAUDE.md의 Windows 경로를 사용한다.

```bash
git fetch -q origin
git ls-remote --heads origin <포켓몬>    # 출력이 비어 있어야 사용 가능

WT=~/.worktrees/brand/<관광명소>
git worktree add "$WT" -b <포켓몬> origin/main

# 버전관리 안 되는 설정 파일을 옮긴다. 없는 파일은 건너뛴다.
for f in .env .env.local src/main/resources/application-local.yaml; do
  [ -f "$f" ] || continue
